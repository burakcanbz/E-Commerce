...
import { getRandomText } from "../../utils/helpers.ts";
...
const ProductCard = React.memo(
  ({ product, searchedProduct }: ProductCardPropsType) => {
    ...
    useEffect(() => {
      localStorage.setItem(
        "cart",
        JSON.stringify({ cartItems, shippingAddress, paymentMethod })
      );
    }, [cartItems, shippingAddress, paymentMethod]);

    return (
      <Card
        className="product-card my-3 p-2 rounded shadow-lg"
        style={{
          minWidth: 180,
          maxWidth: 180,
          maxHeight: 350,
      ...
      >
        <Link to={`/product/${p._id}`} target="_blank">
          ...
          <div className="card-text-container">
              <Card.ImgOverlay
              style={{ color: randomText === "Limited Offer" ? "#ec7c13ff": randomText === "New Arrival" ? "#6996dfff" : "black"}}
                className={randomText !== "Best Seller" ? "card-text-inner-contents-animation" : "card-text-inner-contents"}>
                {randomText}
              </Card.ImgOverlay>
          </div>
        </Link>
        ...
        <Card.Body
          style={{ marginTop: -25, marginLeft: -14, marginRight: -14 }}
        >
          <Link
            to={`/product/${p._id}`}
          ...
          >
            ...
            <button
              ...
              disabled={
                p.countInStock === 0 ||
                (productExist?.qty !== undefined &&
                  productExist?.qty >= p.countInStock)
              }
            >
